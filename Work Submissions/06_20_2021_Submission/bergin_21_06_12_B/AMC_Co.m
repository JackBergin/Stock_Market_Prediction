...
print('AMCSmoothedVHistorical','-dpdf') % create pdf at end
hold
pause
...
title('Closing Stock Prices for AMC')
pause
subplot(3,2,2)
autocorr(data,m-1)
pause
%the auto corrlation hit zero at day 173 in the past
%the relevant data is between day 332 and day 505 (=today) 
RV=prclose(332:505);
rp=174;%relevant period
yy=linspace(1,rp,rp);
subplot(3,2,3)
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('AMC closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for AMC with Trendline')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for AMC with Fourier Model')
hold
pause
...
hold off
print('AMC','-dpdf') % create pdf at end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOW to the predictions
load('AMCFuture.csv');%load the future data
[m,n]=size(AMCFuture)%find the numbet of days in the data
...
title('AMC Prototype model-trend+Fourier')
legend('model','closing prices','errorband'),
hold
print('AMCFuture','-dpdf') % create pdf at end
