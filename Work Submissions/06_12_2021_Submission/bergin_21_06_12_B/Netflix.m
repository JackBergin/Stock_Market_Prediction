...
print('NFLXSmoothedVHistorical','-dpdf') % create pdf at end
hold
pause
...
title('Closing Stock Prices for NFLX')
pause
subplot(3,2,2)
autocorr(data,m-1)
pause
...
RV=prclose(343:505);
rp=163;%relevant period
yy=linspace(1,rp,rp);
subplot(3,2,3)
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('NFLX closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for NFLX with Trendline')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for NFLX with Fourier Model')
hold
pause
...
hold off
print('NFLX','-dpdf') % create pdf at end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOW to the predictions
load('NFLXFuture.csv');%load the future data
[m,n]=size(NFLXFuture)%find the numbet of days in the data
...
title('NFLX Prototype model-trend+Fourier')
legend('model','closing prices','errorband'),
hold
print('NFLXFuture','-dpdf') % create pdf at end
