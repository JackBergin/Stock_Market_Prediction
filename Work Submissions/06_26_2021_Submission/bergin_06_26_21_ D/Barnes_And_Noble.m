clear all
load ('BNED.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=BNED(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "BNED.csv" is arranged so the first row is the oldest data
...
title('Closing Stock Prices for BNED')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(412:505);
rp=94;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('BNED closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for BNED with Trendline')
print('BNED01','-dpdf')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for BNED with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('BNED Prototype model-trend+Fourier')
...
load('BNEDfuture.csv');%load txhe future data
[m,n]=size(BNEDfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=BNEDfuture(i,5)/today_price;
end
...
title('BNED Prototype+Market influence')
