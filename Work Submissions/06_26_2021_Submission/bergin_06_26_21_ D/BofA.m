clear all
load ('BAC.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=BAC(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "BAC.csv" is arranged so the first row is the oldest data
...
title('Closing Stock Prices for BAC')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(426:505);
rp=80;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('BAC closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for BAC with Trendline')
print('BAC01','-dpdf')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for BAC with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('BAC Prototype model-trend+Fourier')
...
load('BACfuture.csv');%load txhe future data
[m,n]=size(BACfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=BACfuture(i,5)/today_price;
end
...
title('BAC Prototype+Market influence')
