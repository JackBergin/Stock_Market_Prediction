clear all
load ('BTC_VIX_Fit.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=BTC_VIX_Fit(:,5);%the closing price for each day is in the fifth col. 
...
title('Closing Stock Prices for BTC')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(328:505);
rp=178;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('BTC closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for BTC with Trendline')
print('BTC01','-dpdf')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for BTC with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('BTC Prototype model-trend+Fourier')
...
load('BTCfuture.csv');%load txhe future data
[m,n]=size(BTCfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=BTCfuture(i,5)/today_price;
end
...
title('BTC Prototype+Market influence')
