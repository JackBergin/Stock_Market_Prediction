...
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=SONY(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "SONY.csv" is arranged so the first row is the oldest data
...
load('SONYfuture.csv');%load txhe future data
...
for i=1:m
RV1(i+rp)=SONYfuture(i,5)/today_price;
end
...
title('SONY Prototype+Market influence')
