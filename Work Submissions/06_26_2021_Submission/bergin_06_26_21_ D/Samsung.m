clear all
load ('SAMSNG.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=SAMSNG(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "SAMSNG.csv" is arranged so the first row is the oldest data
% while "today" is the last row. We need to inver this
[m,n]=size(prclose)%how many data entries we have
pause
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%normalize the data
today_price=prclose(495);
prclose=prclose/prclose(495);
VIXclose=VIXclose/VIXclose(505);
bdata=prclose;
ddata=VIXclose;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%TREAT SAMSNG STOCK
...
title('Closing Stock Prices for SAMSNG')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(363:495);
rp=133;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('SAMSNG closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for SAMSNG with Trendline')
print('SAMSNG01','-dpdf')
hold
pause
...
ylabel('Closing Price')
title('Closing Stock Prices for SAMSNG with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('SAMSNG Prototype model-trend+Fourier')
...
load('SAMSNGfuture.csv');%load txhe future data
[m,n]=size(SAMSNGfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=SAMSNGfuture(i,5)/today_price;
end
...
title('SAMSNG Prototype+Market influence')
