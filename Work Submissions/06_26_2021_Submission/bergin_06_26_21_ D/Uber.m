clear all
load ('UBER.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=UBER(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "UBER.csv" is arranged so the first row is the oldest data
...
title('Closing Stock Prices for UBER')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(388:505);
rp=118;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('UBER closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for UBER with Trendline')
print('UBER01','-dpdf')
hold
pause
...
ylabel('Closing Price')
title('Closing Stock Prices for UBER with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('UBER Prototype model-trend+Fourier')
...
load('UBERfuture.csv');%load txhe future data
[m,n]=size(UBERfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=UBERfuture(i,5)/today_price;
end
...
title('UBER Prototype+Market influence')
