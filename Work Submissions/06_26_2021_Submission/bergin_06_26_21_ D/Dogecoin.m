clear all
load ('DOGE_VIX_Fit.csv');%the file containing the historical data  for this stock
load ('VIX.csv');%the file containing the historical data  for VIX in the last year
VIXclose=VIX(:,5);
[m1,n1]=size(VIXclose)
prclose=DOGE_VIX_Fit(:,5);%the closing price for each day is in the fifth col. 
%prclose is a vector containing the historical closing price data
% the data in "DOGE.csv" is arranged so the first row is the oldest data
...
title('Closing Stock Prices for DOGE')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(421:505);
rp=85;%relevant period
yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('DOGE closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for DOGE with Trendline')
print('DOGE01','-dpdf')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for DOGE with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('DOGE Prototype model-trend+Fourier')
...
load('DOGEfuture.csv');%load txhe future data
[m,n]=size(DOGEfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=DOGEfuture(i,5)/today_price;
end
...
title('DOGE Prototype+Market influence')
