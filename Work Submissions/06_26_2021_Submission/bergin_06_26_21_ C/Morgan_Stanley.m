...
clear all
load ('MS.csv');%the file containing the historical data  for this stock
load ('DJI.csv');%the file containing the historical data  for DJI in the last year
DJIclose=DJI(:,5);
[m1,n1]=size(DJIclose)
prclose=MS(:,5);%the closing price for each day is in the fifth col. 
%prclse is a vector containing the historical closing price data
% the data in "MS.csv" is arranged so the first row is the oldest data
...
title('Closing Stock Prices for MS')
pause
autocorr(bdata,m-1)
pause
...
RV=prclose(357:505);
rp=149;%relevant period

yy=linspace(1,rp,rp);
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('MS closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for MS with Trendline')
print('MS01','-dpdf')
hold
pause
...
ylabel('Closing Price')
title('Closing Stock Prices for MS with Fourier Model')
hold
pause
...
plot(yy,yh-width,'k')
xlabel('Days in relevant period')
title('MS Prototype model-trend+Fourier')
...
load('MSfuture.csv');%load txhe future data
[m,n]=size(MSfuture)%find the numbet of days in the data
...
for i=1:m
RV1(i+rp)=MSfuture(i,5)/today_price;
end
...
title('MS Prototype+Market influence')
