...
print('NSRGYSmoothedVHistorical','-dpdf') % create pdf at end
hold
pause
...
title('Closing Stock Prices for NSRGY')
pause
subplot(3,2,2)
autocorr(data,m-1)
pause
...
RV=prclose(306:505);
rp=200;%relevant period
yy=linspace(1,rp,rp);
subplot(3,2,3)
plot(yy,RV)
xlabel('days')
ylabel('closing price')
title('NSRGY closing prices for the relevant period')
...
ylabel('Closing Price')
title('Closing Stock Prices for NSRGY with Trendline')
hold
pause
...
xlabel('Day number starting 05/26/2019')
ylabel('Closing Price')
title('Closing Stock Prices for NSRGY with Fourier Model')
hold
pause
...
hold off
print('NSRGY','-dpdf') % create pdf at end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOW to the predictions
load('NSRGYFuture.csv');%load the future data
[m,n]=size(NSRGYFuture)%find the numbet of days in the data
...
title('NSRGY Prototype model-trend+Fourier')
legend('model','closing prices','errorband'),
hold
print('NSRGYFuture','-dpdf') % create pdf at end
